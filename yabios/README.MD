...
- [x] [`_far_memcpy`](https://github.com/feilipu/yaz180/tree/master/yabios#_far_memcpy), [`_far_memset`](https://github.com/feilipu/yaz180/tree/master/yabios#_far_memset) functions
...
- [ ] [`RST+DEFB` APU Call](https://github.com/feilipu/yaz180/tree/master/yabios#rst28defb-apu-call)
...
- [x] [`RST+DEFW+DEFB` `_far_call`](https://github.com/feilipu/yaz180/tree/master/yabios#rst10defwdefb-_far_call)
- [x] [`RST` `_far_jp`](https://github.com/feilipu/yaz180/tree/master/yabios#rst18-_far_jp)
- [ ] finalise and test
- [ ] to be added
...
The basic layout is to allow for an initial boot from flash memory into a system `BANK0`, with additional `BANK1` through `BANK12` containing 64kB RAM based application spaces. The upper 4kB of each application space will be masked by `COMMON AREA 1` RAM, which provides system utilities.
...
For all far memory functions, the high byte of the 20bit address will be a twos complement bank-relative address. Additionally, as a later enhancement, calling far memory functions from a non-zero bank (as a user) will ensure that `BANK0` and `BANK13`, `BANK14`, and `BANK15` cannot be reached, and `COMMON_AREA_1` memory cannot be written.
...
`far void *far_memcpy(far void *str1, const far void *str2, size_t n)`<br>
`str1` − This is a pointer to the destination array where the content is to be copied, type-cast to a pointer of type `far void*`.<br>
`str2` − This is a pointer to the source of data to be copied, type-cast to a pointer of type `far void*`.<br>
`n` − This is the number of bytes to be copied.<br>
This function returns a _relative_ `far void*` to destination, which is `str1` in `EHL`.
...
`far void *far_memset(far void *str, int c, size_t n)`<br>
...
This function returns a _relative_ `far void*` pointer to the memory area `str` in `EHL`.
...
When the user calls `_load_hex`, access to `BANK0` down through `BANK13` will be prevented, enabling relocation of applications, and will ensure that critical `PAGE0` and `COMMON AREA 1` memory cannot be overwritten.
...
YABIOS `_far_jp` call are provided to support an application in its own memory space. The location of the application `_user_function` is provided by a `far void *`.

The `_far_jp` call is generated by following example byte code:
...
The alternate register set is modified only during the internal bank switch with interrupts disabled, during a System Call. This is only a momentary situation. Normal operation is with interrupts enabled.
...
YABIOS `_far_call` calls are provided to support an application in its own memory space. The location of the application `_user_function` is provided by a `far void *`.

The called `_user_function` parameters are placed on the Stack or Registers, and the `_far_call` call is generated by following example byte code:
