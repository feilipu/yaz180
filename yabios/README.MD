...
- [ ] [`RST+DEFW+DEFB` `_far_jp`](https://github.com/feilipu/yaz180/tree/master/yabios#rst18defwdefb-_far_jp)
- [ ] finalise and test
- [ ] to be added
...
YABIOS `_far_jp` calls are provided to support an application in its own memory space. The location of the application `_user_function` is provided by a `far_void*`.

Checking is done to prevent the user `_far_jp` call reaching within the `BANK0`, or `COMMON AREA 1` memory space, to protect the system integrity.
...
Checking is done to prevent the user `_far_call` call reaching within the `BANK0`, or `COMMON AREA 1` memory space, to protect the system integrity.
